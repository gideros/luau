...
#define VM_DISPATCH_TABLE() \
    VM_DISPATCH_OP(LOP_NOP), VM_DISPATCH_OP(LOP_BREAK), VM_DISPATCH_OP(LOP_LOADNIL), VM_DISPATCH_OP(LOP_LOADB), VM_DISPATCH_OP(LOP_LOADN), \
        ...
        VM_DISPATCH_OP(LOP_IDIVK), VM_DISPATCH_OP(LOP_MINOF), VM_DISPATCH_OP(LOP_MAXOF), \
        ...
        VM_DISPATCH_OP(LOP_MINOFK), VM_DISPATCH_OP(LOP_MAXOFK), \
...
//GIDEROS
            VM_CASE(LOP_MAXOF)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_MAXOF>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_MINOF)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_MINOF>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_MAXOFK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_nummax(nb, nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_MAXOF>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_MINOFK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_nummin(nb, nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_MINOF>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINAND)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BAND>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINOR)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BOR>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINXOR)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BXOR>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_SHIFTR)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_SHR>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_SHIFTL)
            {
                ...
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_SHL>(L, ra, rb, rc));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINANDK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_numband(nb,nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BAND>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINORK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_numbor(nb,nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BOR>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINXORK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_numbxor(nb,nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BXOR>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_SHIFTRK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_numbsr(nb,nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_SHR>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_SHIFTLK)
            {
                ...
                if (ttisnumber(rb))
                {
                    double nb = nvalue(rb);
                    double nk = nvalue(kv);
                    setnvalue(ra, luai_numbsl(nb,nk));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_SHL>(L, ra, rb, kv));
                    VM_NEXT();
                }
            }
            ...
            VM_CASE(LOP_BINNOT)
            {
                ...
                if (ttisnumber(rb))
                {
                    setnvalue(ra, luai_numbnot(nvalue(rb)));
                    VM_NEXT();
                }
                else
                {
                    // slow-path, may invoke C/Lua via metamethods
                    VM_PROTECT(luaV_doarithimpl<TM_BNOT>(L, ra, rb, rb));
                    VM_NEXT();
                }
            }
