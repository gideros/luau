...
void luaV_getimport(lua_State *L, LuaTable *env, TValue *k, StkId res,
...
static void resolveImportSafe(lua_State *L, LuaTable *env, TValue *k,
		uint32_t id) {
	struct ResolveImport {
		...
		static void run(lua_State *L, void *ud) {
			...
			luaV_getimport(L, L->gt, self->k, L->top - 1, self->id, /* propagatenil= */
					true);
		}
	};
	...
	if (L->gt->safeenv & LUA_SAFEENV_IMPORTS) {
		...
		int status = luaD_pcall(L, &ResolveImport::run, &ri,
				savestack(L, L->top), 0);
	...
	if (localCount != 0) {
		for (uint32_t i = 0; i < localCount; i++) {
			uint32_t index = uint32_t(
					data[offset] - LBC_TYPE_TAGGED_USERDATA_BASE);
...
static int loadsafe(lua_State *L, TempBuffer<TString*> &strings,
		TempBuffer<Proto*> &protos, const char *chunkname, const char *data,
		size_t size, int env) {
	size_t offset = 0;
	size_t done = 0;
	...
	while (offset < size) {
		...
		if (version == 0) {
			...
			const char *chunkid = luaO_chunkid(chunkbuf, sizeof(chunkbuf),
					chunkname, strlen(chunkname));
			lua_pushfstring(L, "%s%.*s", chunkid, int(size - offset),
					data + offset);
			return 1;
		}
		...
		if (version >= 4) {
			...
			if (typesversion < LBC_TYPE_VERSION_MIN
				...
				lua_pushfstring(L,
						...
						chunkid, LBC_TYPE_VERSION_MIN, LBC_TYPE_VERSION_MAX,
						typesversion);
				return 1;
			}
		}
		...
		const uint32_t userdataTypeLimit = LBC_TYPE_TAGGED_USERDATA_END
			...
			while (index != 0) {
				...
				if (uint32_t(index - 1) < userdataTypeLimit) {
					if (auto cb = L->global->ecb.gettypemapping)
						userdataRemapping[index - 1] = cb(L, getstr(name),
								name->len);
				}
		...
		for (unsigned int i = 0; i < protoCount; ++i) {
			...
			if (version >= 4) {
				...
				if (typesversion == 1) {
					...
					if (typesize) {
						...
						p->typeinfo = luaM_newarray(L, headersize + typesize,
								uint8_t, p->memcat);
					...
					if (typesize) {
						...
						p->typeinfo = luaM_newarray(L, typesize, uint8_t,
								p->memcat);
						...
						if (typesversion == 3) {
							remapUserdataTypes((char*) (uint8_t*) p->typeinfo,
									p->sizetypeinfo, userdataRemapping,
									userdataTypeLimit);
						}
					}
				}
			...
			for (int j = 0; j < p->sizek; ++j) {
				...
				case LBC_CONSTANT_CLOSURE: {
					...
					Closure *cl = luaF_newLclosure(L, protos[fid]->nups, envt,
							protos[fid]);
			...
			if (lineinfo) {
				...
				p->lineinfo = luaM_newarray(L, sizelineinfo, uint8_t,
						p->memcat);
			...
			if (debuginfo) {
				...
				for (int j = 0; j < p->sizelocvars; ++j) {
					p->locvars[j].varname = readString(strings, data, size,
							offset);
				...
				p->upvalues = luaM_newarray(L, sizeupvalues, TString*,
						p->memcat);
...
int luau_load(lua_State *L, const char *chunkname, const char *data,
	...
	struct LoadContext {
		...
		static void run(lua_State *L, void *ud) {
			...
			ctx->result = loadsafe(L, ctx->strings, ctx->protos, ctx->chunkname,
					ctx->data, ctx->size, ctx->env);
		}
	} ctx = { { }, { }, chunkname, data, size, env, };
